...
class H {
  ...
  public static function buildMenuListItems($menu,$dropdownClass=""){
    ...
    foreach($menu as $key => $val):
      ...
      if(is_array($val)): ?>
        <li class="nav-item dropdown middle-grid">
          <a href="#" class="nav-link dropdown-toggle greeting" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?=$key?></a>
      ...
      <?php else:
        ...
        <li class="nav-item"><a class="nav-link header-links<?=$active?>" href="<?=$val?>"><?=strtoupper($key)?></a></li>
      <?php endif; ?>
    <?php endforeach;
    return ob_get_clean();
  }
