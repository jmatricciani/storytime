<?php use App\Lib\Elements\FrontEnd;
      use Core\{FH};
?>
<?php $this->setSiteTitle($this->tape->name); ?>
<?php $this->start('body') ?>

  <span class="br-row"></span>
  <h1 class="kanit-bold heading"><?=$this->tape->name?></h1>
  <span class="br-head"></span>
  <div class="grid-2">
    <div class="image-block">
      <img class="full-image" src="<?=PROOT.$this->tape->imageUrl?>" alt="<?=$this->tape->name?>">
    </div>
    <div class="description-block">
      <h1 class="kanit-medium sub-heading">Tape Release</h1>
      <h1 class="roboto-light body-heading">by</h1>
      <h1 class="kanit-bold heading"><?=$this->artist_name?></h1>
      <p class="roboto-light body"><?=html_entity_decode($this->tape->body)?></p>
      <h1 class="roboto-light body">Uploaded:<br/><?=$this->tape->created_at?></h1>
    </div>
  </div>
  <span class="br-row"></span>
  <?php if($this->inventory->stock): ?>
    <h1 class="kanit-bold stock-display text-center"><?=$this->inventory->stock?> in stock</h1>
  <?php else: ?>
  ...
  <form action="<?=PROOT?>cart/addToCart" method="post">
    <?=FH::csrfInput()?>
    <?=FrontEnd::addToCart($this->tape->price,$this->tape->shipping); ?>
