...
<?php $this->setSiteTitle($this->mix->name); ?>
<?php $this->start('body') ?>

  <span class="br-row"></span>
  <h1 class="kanit-bold heading"><?=$this->mix->name?></h1>
  <span class="br-head"></span>
  <div class="grid-2">
    <div class="image-block">
      <img class="full-image" src="<?=PROOT.$this->mix->imageUrl?>" alt="<?=$this->mix->name?>">
    </div>
    <div class="description-block">
      <h1 class="kanit-medium sub-heading">Mix</h1>
      <h1 class="roboto-light body-heading">by</h1>
      <h1 class="kanit-bold heading"><?=$this->artist_name?></h1>
      <p class="roboto-light body"><?=html_entity_decode($this->mix->body)?></p>
      <h1 class="roboto-light body">Uploaded:<br/><?=$this->mix->created_at?></h1>
    </div>
  </div>
  <input type="hidden" id="mix" audiourl="<?=PROOT.$this->mix->audioUrl?>" cover="<?=PROOT.$this->mix->imageUrl?>" title="<?=$this->mix->name?>" artist="<?=$this->artist_name?>"/>
  <span class="br-row"></span>
  ...
  function rangeSlider(id, onDrag) {
    ...
    jQuery(document).ready(function() {
      ...
      function initAudio() {
          var elem = document.querySelector('input[id="mix"]');
