...
<div id="sortableImages" class="row align-items-center justify-content-start p-2">
    <?php foreach($this->images as $image):?>
      <div class="col flex-grow-0 sortableImage" id="image_<?=$image->id?>">
...
<script>
  ...
  function deleteImage(image_id){
    if($('#sortableImages .sortableImage').length == 1){
      alertMsg('Cannot delete the last image.','danger');
    } else {
  ...
  jQuery('document').ready(function(){
    jQuery( "#sortableImages" ).sortable({
      ...
      update: function( event, ui ) {
        updateSort();
        $("#sortableImages").sortable("refreshPositions");
      }
    });
